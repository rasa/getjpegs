...
//------------------------------------------------------------------------------
// Read a Huffman code table. (Define Huffman Tables) marker.
...
Lq: (16bits) Quantization table definition length – Specifies the length of all quantization table parameters shown in

Pq: (4bits) Quantization table element precision – Specifies the precision of the Qk values. Value 0 indicates 8-bit Qk
values; value 1 indicates 16-bit Qk values. Pq shall be zero for 8 bit sample precision P (see B.2.2).

Tq: (4bits) Quantization table destination identifier – Specifies one of four possible destinations at the decoder into
which the quantization table shall be installed.

Qk: (8or16bits - depends on Pq) Quantization table element – Specifies the kth element out of 64 elements, where k is the index in the zigzag
...
{
  xmlStream_ << "<quantizationTable>\n";

  printf("Reading the quantization table.\n");
  int n, i, prec;
  uint left;
  uint temp;
  ...
  /* the table def length is the length of the table, plus the precision and table number. */
  /* so, will probably be 65 or 129. */

  const uint quantizationTableDefinitionLength = left;
  printf("Quantization Table Def Len %d bytes.\n", quantizationTableDefinitionLength);
  ...
  while (left)
  {
    ...
    prec = n >> 4;
    n &= 0x0F;
    const uint quantizationTableElementPrecision = prec; /* indicates 8bit or 16bit precision */
    const uint quantizationTableNumber = n;

    //printf("quantization Table no:%d\n", quantizationTableNumber);
    xmlStream_ << "<tableNo>" << quantizationTableNumber << "</tableNo>\n";
    //printf("qunatization tables have %d bits of precision\n", quantizationTableElementPrecision == 0 ? 8 : 16);
    xmlStream_ << "<bitsOfPrecision>" << (quantizationTableElementPrecision == 0 ? 8 : 16) << "</bitsOfPrecision>\n";
    
    if (n >= JPGD_MAXQUANTTABLES) {
      terminate(JPGD_BAD_DQT_TABLE);
    }
    /* allocate memory for quantization table */
    //printf("Quantization table size %d bytes.\n", 64 * sizeof(QUANT_TYPE));
    xmlStream_ << "<sizeInBytes>" << (64 * sizeof(QUANT_TYPE)) << "</sizeInBytes>\n";

    if (!quant[n]) {
      quant[n] = (QUANT_TYPE *)alloc(64 * sizeof(QUANT_TYPE));
    }
    // read quantization entries, in zag order
    xmlStream_ << "<tableData>\n";
    for (i = 0; i < 64; i++)
    {
      temp = get_bits_1(8);

      if (prec) /* if 16bit precision, get another 8 bits */
...
void jpeg_decoder::read_sof1_marker(void)
{
  xmlStream_ << "<startOfFrame>\n";
  ...
  xmlStream_ << "<imageDimensions>" << image_x_size << "x" << image_y_size << "</imageDimensions>\n";

  if ((image_x_size < 1) || (image_x_size > JPGD_MAX_WIDTH))
    terminate(JPGD_BAD_WIDTH);

  comps_in_frame = get_bits_1(8);
  xmlStream_ << "<componentsInFrame>" << comps_in_frame << "</componentsInFrame>\n";
  ...
  xmlStream_ << "<components>\n";
  for (i = 0; i < comps_in_frame; i++)
  {
  xmlStream_ << "<component component_no=\"" << i << "\">\n";
...
*/
    ...
    xmlStream_ << "<componentIdentifier>" << comp_ident[i] << "</componentIdentifier>\n";
...
*/
    ...
    xmlStream_ << "<horizontalSamplingFactor>" <<  comp_h_samp[i] << "</horizontalSamplingFactor>\n";
...
*/
    ...
    xmlStream_ << "<verticalSamplingFactor>" <<  comp_v_samp[i] << "</verticalSamplingFactor>\n";
...
by the time the decoder is ready to decode the scan(s) containing component Ci. The destination shall not be re-
specified, or its contents changed, until all scans containing Ci have been completed.
*/
  comp_quant[i]  = get_bits_1(8);
  xmlStream_ << "<quantizationTableDestinationSelector>" <<  comp_quant[i] << "</quantizationTableDestinationSelector>\n";

  xmlStream_ << "</component>\n";
...
void jpeg_decoder::read_sos_marker(void)
{
  ...
  comps_in_scan = n; /* number of components. if more than 1, then it will be interleaved */
...
void jpeg_decoder::locate_sof_marker(void)
{
  ...
  switch (c)
  {
    ...
    case M_SOF1:  /* extended sequential DCT */ /* SOF1: Extended sequential DCT, Huffman coding */
    {
      xmlStream_ << "<progressive>" << (progressive_flag ? "true" : "false") << "</progressive>\n";
...
void jpeg_decoder::init(Pjpeg_decoder_stream Pstream, const bool use_mmx)
{
  error_code = 0;
...
void jpeg_decoder::decode_next_row(void)
{
  ...
  for (int mcu_row = 0; mcu_row < mcus_per_row; mcu_row++) // for_each mcu on this row
  {
    if ((restart_interval) && (restarts_left == 0))
      process_restart();

    for (int mcu_block = 0; mcu_block < blocks_per_mcu; mcu_block++) // for each block in this mcu.
    {
      int component_id = mcu_org[mcu_block];
      ...
      /* RLE on AC components - On the individual entries in the 64-element vector (the AC components), 
      a Run Length Encoding stores each value along with the number of zeros preceeding it. 
      As the 1x64 vector contains a lot of zeros, it is more efficient to save the non-zero 
      values and then count the number of zeros between these non-zero values. The 
      RLE stores a skip and a value, where skip is the number of zeros before this component, 
      and the value is the next non-zero component.
      */
...
void jpeg_decoder::decode_init(Pjpeg_decoder_stream Pstream, const bool use_mmx)
{
...
jpeg_decoder::jpeg_decoder(Pjpeg_decoder_stream Pstream, std::ofstream& xmlStream, const bool use_mmx) : 
  xmlStream_(xmlStream)
{
  if (setjmp(jmp_state))
    return;
